    ...
    class Program
    {
        ...
        private static async Task RunAsync(ConfigWrapper config)
        {
            ...
            try
            {
                ...
                {
                    var cencKey = PallyConHelper.GetCencKeyFromPallyCon(config.PallyConKmsUrl, config.PallyConEncToken, ContentId);
                    cencKey.LabelReferenceInStreamingPolicy = LabelDefaultKey;
                    ...
                    StreamingPolicy streamingPolicy = await GetOrCreateStreamingPolicyAsync(client, config.ResourceGroup, config.AccountName
                        , StreamingPolicyName, config.PallyConLicenseUrl);
                    ...
                    StreamingLocator streamingLocator = await CreateStreamingLocatorAsync(client, config.ResourceGroup, config.AccountName, outputAsset.Name
                        , locatorName, streamingPolicy.Name, ContentId, cencKey);
                    ...
                    string dashPath = await GetDASHStreamingUrlAsync(client, config.ResourceGroup, config.AccountName, streamingLocator.Name, streamingEndpoint);

                    Console.WriteLine();
                    Console.WriteLine("DASH url can be played on Chrome/Firefox");
                    Console.WriteLine(dashPath);
                    Console.WriteLine();
                }
        ...
        /// <summary>
        /// Get or create a custom streaming policy.
        /// </summary>
        /// <param name="client">The Media Services client.</param>
        /// <param name="resourceGroupName">The name of the resource group within the Azure subscription.</param>
        /// <param name="accountName"> The Media Services account name.</param>
        /// <param name="streamingPolicyName">The streaming policy name.</param>
        /// <param name="licenseUrl">The license acquisition url.</param>
        ...
        private static async Task<StreamingPolicy> GetOrCreateStreamingPolicyAsync(IAzureMediaServicesClient client,
            string resourceGroupName, string accountName, string streamingPolicyName, string licenseUrl)
        {
                ...
                streamingPolicy = new StreamingPolicy
                {
                    CommonEncryptionCenc = new CommonEncryptionCenc()
                    {
                        Drm = new CencDrmConfiguration()
                        {
                            ...
                            {
                                CustomLicenseAcquisitionUrlTemplate = licenseUrl
                            }
                            ,
        ...
        /// <param name="cencKey">The content key.</param>
        /// <returns></returns>
        private static async Task<StreamingLocator> CreateStreamingLocatorAsync(
            ...
            string contentId,
            StreamingLocatorContentKey cencKey)
        {
            ...
            var customLocator = new StreamingLocator(
                ...
                contentKeys: new List<StreamingLocatorContentKey> { cencKey },
        ...
        private static async Task CleanUpAsync(
            ...
            string PallyConStreamingPolicyName
            )
        {
            ...
            await client.StreamingPolicies.DeleteAsync(resourceGroupName, accountName, PallyConStreamingPolicyName);
